...
public class AppointmentController extends ExceptionResolver {
	...
	public APIresponse setAppointment(@Valid @RequestBody AppointmentDummy appointmentDummy,
			...
			for (AppointmentTime time : times) {
				...
				if (Duration.between(sTime, eTime).toHours() > 8) {
					return new APIresponse(HttpStatus.FORBIDDEN.value(),
							"Sorry! The interval between start time and end time of your Appointment cannot be more than 8 hours! Please consider adding another time!",
							null);
				}
		...
		if (timeSlots.size() < emailFromDummy.size()) {
			return new APIresponse(HttpStatus.FORBIDDEN.value(),
					"Sorry! The generated Appointment Slots are not enough for provided numbers of Recepients! Please add more time slots or reduce number of recepients!",
					null);
		}
...
//		}
	...
	public APIresponse getFacultyAppointments() {
		...
		if (appointments == null) {
			return new APIresponse(HttpStatus.NOT_FOUND.value(), "You have not created any appointments yet.", null);
		}
		...
		for (Appointment app : appointments) {
			...
			allAppointments.add(new AppointmentResponse(app.getId(), app.getName(), app.getDescription(),
					app.getLocation(), responseDate));
	...
	public APIresponse getSlots(@PathVariable("id") Long appointmentId) {
		...
		if (appointment == null) {
			return new APIresponse(HttpStatus.NOT_FOUND.value(),
					"The appointment with id " + appointmentId + " does not exist.", null);
		}
	...
	public APIresponse getSlotsForFaculty(@PathVariable("id") Long appointmentId) {
		...
		List<DateAndTimeSlotResponse> response = new ArrayList<DateAndTimeSlotResponse>();
		// ResponseWithPendingUser finalResponse = new
		// ArrayList<ResponseWithPendingUser>();
	...
	public APIresponse postSlots(@PathVariable("timeSlotId") long id) {
		...
		if (slotToRemove == null) {
			if (slotToRemove == null) {
				return new APIresponse(HttpStatus.NOT_FOUND.value(), "The time slot with id " + id + " does not exist.",
						null);
			}
		}
	...
	public APIresponse sendToCalendar(@Valid @PathVariable("appointmentId") long id) {
		...
		if (appointment == null) {
			return new APIresponse(HttpStatus.NOT_FOUND.value(), "The appointment with id " + id + " does not exist.",
					null);
	...
	// sends all the timeslots to admin/creator of appointment that are already
	// selected by receipients
	...
	public APIresponse scheduledAppointmentsByUser() {
		...
		if (allAppointments == null) {
			return new APIresponse(HttpStatus.NOT_FOUND.value(),
					"You have not created any appointment! Please create appointment and assign appointees!", null);
		} else {
			...
			if (slotResponses == null) {
				return new APIresponse(HttpStatus.NOT_FOUND.value(),
						"Appointment reciepient has not selected their choices of timeslot for created appointments! Wait before schduled Appointments can be seen",
						null);
			} else {
	...
	public APIresponse scheduledAppointments() {
		...
		if (appointmentService.findAllByRecepients(currentUser) == null) {
			return new APIresponse(HttpStatus.NOT_FOUND.value(),
					"You have not recieved any appointment at the moment!!.", null);
		}
	...
	@PreAuthorize("hasRole('PM') or hasRole('ADMIN')")
	public APIresponse editAppointment(@Valid @RequestBody AppointmentEdit appointmentEdit,
		...
		if (appointment.getCreatedBy() != user) {
			return new APIresponse(HttpStatus.FORBIDDEN.value(),
					"You did not create the appointment. Authorization denied!", null);
		}
		...
		if (recipientsCounter > timeSlotCounter) {
			return new APIresponse(HttpStatus.BAD_REQUEST.value(),
					"The number of recipients exceeds the number of timeslots. Please remove one or more recipients.",
					null);
		}
			...
			if (!newRecipients.contains(oldRecipient)) {
				...
				for (TimeSlots slots : timeSlots) {
					if (slots.getSelectedBy() == oldRecipient) {
						return new APIresponse(HttpStatus.FORBIDDEN.value(),
								slots.getSelectedBy().getName() + " has already selected a time slot from "
										+ slots.getStartTime() + " to " + slots.getEndTime()
										+ ". You cannot remove this person from the appointment.",
								null);
					}
				}
...
//		}
	...
	public APIresponse deleteAppointment(@PathVariable("id") Long appointmentId) {
		...
		if (appointment == null) {
			return new APIresponse(HttpStatus.NOT_FOUND.value(),
					"Appointment with id " + appointmentId + " does not exists.", null);
		}
